...
os.environ['LANGCHAIN_PROJECT'] = 'TracebleRAGChatbot'
...
# used to cache the given file so that latency can be increased

"""
Flow:
- if indices are not created then it'll created and stored locally
- if indices are present then they will be referred

conditions when it'll build the indices:
1. no cache exist
2. pdf content changes
3. pdf metadata changes
4. chunking params changes (eb. chunk size, chunk overlap)
...
def load_or_build_index(
    ...
    embed_model_name: str = "embed-english-v3.0",
...
def setup_pipeline(pdf_path: str, chunk_size=1000, chunk_overlap=150, embed_model_name="embed-english-v3.0", force_rebuild=False):
